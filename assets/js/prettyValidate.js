...
(function($) {
	...
	$.fn.prettyValidate = function(options) {
		...
		options = $.extend({
			...
			colorChange: null, // fired while colorpicker is changed
		...
		return this.each(function() {
			...
			$('.form-control[type="color"]', form).each(function() {
				...
				$(this).addClass('color').attr('type','text').colourBrightness().colorpicker({
					...
					var color = '';
					if (event == 'create') { color = ($(this).val() && check($(this), 'hex')) ? $(this).val() : '#FFFFFF'; }
					else {
						color = (check($(this), 'hex')) ? event.color.toHex() : '#FFFFFF';
						if ($.isFunction(options.changeColor)) { options.changeColor.call(this, color); }
					}
			...
			form.on('submit', function(event) { var error = 0;
				if ($.isFunction(options.beforeSend)) { options.beforeSend.call(this); }
				input.each(function(){ if (!validate($(this), event)) { error++; } });
				if (error == 0) {
					var valid = {valid: $('.form-control-success', form), event: event};
					if ($.isFunction(options.valid)) { options.valid.call(this, valid); }
					if (options.ajax) {
						var formURL = options.url || form.attr('action'),
						...
						setTimeout(function(){
							$.ajax({
								...
								success: function(data) {
									if ($.isFunction(options.success)) { options.success.call(this, data); }
								...
								error: function(xhr, status, error) {
									if ($.isFunction(options.error)) { options.error.call(this, error); }
									else {
								...
								complete: function() {
									setTimeout(function() {
										if ($.isFunction(options.complete)) { options.complete.call(this); }
					...
					var invalidInput = $('.form-control-danger, .form-control-warning', form);
					var invalid = {empty: $('.form-control-danger', form), invalid: $('.form-control-warning', form)};
					if ($.isFunction(options.invalid)) { options.invalid.call(this, invalid); }
					if (options.shake) {
						invalidInput.parent().addClass('shake');
						setTimeout(function() { invalidInput.parent().removeClass('shake'); }, 500);
					}
					return false;
				}
			});
